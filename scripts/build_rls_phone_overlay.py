...
let root=null,state={ownedVehicles:[],marketplace:{myListings:[],publicListings:[],listingCount:0},players:[]},activeTab=`market`,notice=``,refreshTimer=null;
...
.phone-player-dealer{height:100%;min-height:100%;box-sizing:border-box;padding:14px 14px 22px;overflow-y:auto;color:#f6fff9;background:linear-gradient(180deg,#06140f 0%,#071f18 48%,#02100d 100%);font-family:Overpass,Arial,sans-serif}
.phone-player-dealer *{box-sizing:border-box}
.phone-player-dealer__top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}
.phone-player-dealer__back,.phone-player-dealer__refresh,.phone-player-dealer button{border:0;border-radius:13px;padding:9px 11px;background:#1edc88;color:#03110c;font-weight:900;box-shadow:0 6px 18px rgba(0,0,0,.24)}
.phone-player-dealer__back{background:rgba(255,255,255,.12);color:#fff}
.phone-player-dealer__title{min-width:0;flex:1;text-align:center}
.phone-player-dealer__title h1{margin:0;font-size:19px;letter-spacing:-.04em}
.phone-player-dealer__title p{margin:2px 0 0;color:#a7cabb;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}
.phone-player-dealer__hero{border:1px solid rgba(74,255,178,.25);border-radius:22px;padding:13px;background:linear-gradient(135deg,rgba(34,181,115,.28),rgba(255,255,255,.05));box-shadow:inset 0 1px 0 rgba(255,255,255,.12);margin-bottom:12px}
.phone-player-dealer__hero strong{display:block;font-size:15px;margin-bottom:4px}
.phone-player-dealer__hero span{display:block;color:#c0d8cf;font-size:12px;line-height:1.35}
.phone-player-dealer__tabs{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:12px 0}
.phone-player-dealer__tab{background:rgba(255,255,255,.08)!important;color:#dff8ef!important;border:1px solid rgba(255,255,255,.08)!important;padding:9px 5px!important;font-size:11px!important}
.phone-player-dealer__tab.is-active{background:#1edc88!important;color:#03110c!important}
.phone-player-dealer__notice{margin:8px 0;padding:9px 10px;border-radius:13px;background:rgba(255,255,255,.1);color:#d6fff0;font-size:12px;font-weight:800}
.phone-player-dealer__list{display:flex;flex-direction:column;gap:10px}
.phone-player-dealer__card{border:1px solid rgba(255,255,255,.11);border-radius:18px;background:rgba(7,20,16,.78);padding:12px;box-shadow:0 12px 26px rgba(0,0,0,.2)}
.phone-player-dealer__row{display:flex;justify-content:space-between;align-items:flex-start;gap:9px}
.phone-player-dealer__name{font-size:14px;font-weight:1000;line-height:1.15}
.phone-player-dealer__meta{margin-top:4px;color:#9fbbae;font-size:11px;line-height:1.35}
.phone-player-dealer__price{font-size:14px;font-weight:1000;color:#4dffb5;white-space:nowrap}
.phone-player-dealer__tags{display:flex;flex-wrap:wrap;gap:5px;margin-top:9px}
.phone-player-dealer__tag{border-radius:999px;padding:4px 7px;background:rgba(255,255,255,.1);color:#d9fff2;font-size:10px;font-weight:900}
.phone-player-dealer__tag.warn{background:rgba(255,176,71,.2);color:#ffd797}.phone-player-dealer__tag.good{background:rgba(30,220,136,.2);color:#8dffd0}
.phone-player-dealer__actions{display:flex;gap:7px;margin-top:11px;align-items:center}.phone-player-dealer__actions input{width:100%;min-width:0;border:1px solid rgba(255,255,255,.14);border-radius:13px;padding:9px 10px;background:rgba(255,255,255,.09);color:white;font-weight:900}
...
.phone-player-dealer__empty{text-align:center;border:1px dashed rgba(255,255,255,.18);border-radius:18px;padding:18px 12px;color:#a9c3b8;font-size:13px;line-height:1.35;background:rgba(255,255,255,.04)}
...
function bind(){if(!root)return;root.querySelectorAll(`[data-tab]`).forEach(button=>button.addEventListener(`click`,()=>{activeTab=button.getAttribute(`data-tab`);render()}));root.querySelectorAll(`[data-action]`).forEach(button=>button.addEventListener(`click`,()=>runAction(button.getAttribute(`data-action`),button.getAttribute(`data-id`))));root.querySelectorAll(`input`).forEach(input=>{input.addEventListener(`focus`,()=>engine(`setCEFTyping(true)`));input.addEventListener(`blur`,()=>engine(`setCEFTyping(false)`))});let back=root.querySelector(`[data-back]`);back&&back.addEventListener(`click`,()=>router$1.replace({name:`phone-main`}));let refreshButton=root.querySelector(`[data-refresh]`);refreshButton&&refreshButton.addEventListener(`click`,()=>refresh())}
...
function listingCard(listing,own){return `<article class="phone-player-dealer__card"><div class="phone-player-dealer__row"><div><div class="phone-player-dealer__name">${html(listing.vehicleName||`Vehicle`)}</div><div class="phone-player-dealer__meta">${html(listing.model||`Unknown model`)}${own?``:` • Seller: ${html(listing.sellerName||`Player`)}`}</div></div><div class="phone-player-dealer__price">${money(listing.askingPrice)}</div></div><div class="phone-player-dealer__actions">${own?`<button class="danger" data-action="delist" data-id="${html(listing.listingId)}">Delist</button>`:`<button data-action="buy" data-id="${html(listing.listingId)}">Buy</button>`}</div></article>`}
function vehicleCard(vehicle){let id=String(vehicle.inventoryId||``);let defaultPrice=Number(vehicle.askingPrice||vehicle.marketValue||1000)||1000;return `<article class="phone-player-dealer__card"><div class="phone-player-dealer__row"><div><div class="phone-player-dealer__name">${html(vehicle.vehicleName||`Vehicle`)}</div><div class="phone-player-dealer__meta">${html(vehicle.model||`Unknown model`)} • Est. ${money(vehicle.marketValue)}</div></div></div><div class="phone-player-dealer__tags">${vehicleTags(vehicle)}</div><div class="phone-player-dealer__actions">${vehicle.isListedForSale?`<span class="phone-player-dealer__price">${money(vehicle.askingPrice)}</span><button class="danger" data-action="delist" data-id="${html(vehicle.listingId)}">Delist</button>`:`<input type="number" min="1" step="1" value="${html(defaultPrice)}" data-price-for="${html(id)}"><button data-action="list" data-id="${html(id)}">List</button>`}</div></article>`}
function render(){if(!root)return;ensureStyle();let marketplace=state.marketplace||{},publicListings=array(marketplace.publicListings),myListings=array(marketplace.myListings),owned=array(state.ownedVehicles);let content=``;if(activeTab===`market`)content=publicListings.length?publicListings.map(item=>listingCard(item,false)).join(``):`<div class="phone-player-dealer__empty">No player vehicles are listed right now. When someone opens a dealership listing, it will appear here.</div>`;if(activeTab===`cars`)content=owned.length?owned.map(vehicleCard).join(``):`<div class="phone-player-dealer__empty">No owned vehicles were found in your CareerMP inventory.</div>`;if(activeTab===`mine`)content=myListings.length?myListings.map(item=>listingCard(item,true)).join(``):`<div class="phone-player-dealer__empty">You do not have any active dealership listings yet.</div>`;root.innerHTML=`<div class="phone-player-dealer"><div class="phone-player-dealer__top"><button class="phone-player-dealer__back" data-back>&lt; Home</button><div class="phone-player-dealer__title"><h1>Player Dealer</h1><p>CareerMP marketplace</p></div><button class="phone-player-dealer__refresh" data-refresh>Refresh</button></div><section class="phone-player-dealer__hero"><strong>Open your own dealership.</strong><span>List cars from your garage, browse other players' listings, and buy vehicles without leaving the RLS phone.</span></section><div class="phone-player-dealer__tabs"><button class="phone-player-dealer__tab ${activeTab===`market`?`is-active`:``}" data-tab="market">Market (${publicListings.length})</button><button class="phone-player-dealer__tab ${activeTab===`cars`?`is-active`:``}" data-tab="cars">My Cars (${owned.length})</button><button class="phone-player-dealer__tab ${activeTab===`mine`?`is-active`:``}" data-tab="mine">Mine (${myListings.length})</button></div>${notice?`<div class="phone-player-dealer__notice">${html(notice)}</div>`:``}<div class="phone-player-dealer__list">${content}</div></div>`;bind()}
...
return(_ctx,_cache)=>(openBlock(),createElementBlock(`div`,{class:`phone-player-dealership-native`,ref:el=>root=el},null,512))
}}
"""
